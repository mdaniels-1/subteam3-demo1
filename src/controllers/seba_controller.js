...
async function addPartyToCart(request, response, party_id, user_id){
    ...
    request.on('end', async function(){
        try{
            // Getting User's role
            const user_role = await client.db('Subteam4_Report_3').collection('registered_user_profile').findOne({"user_id": user_id}, {"role": 1, "_id" : 0}).role;
            ...
            // Cart Collection
            const cartCollection = client.db('Subteam4_Report_3').collection('shopping_cart');
        
            // User's current cart
            const current_cart = await cartCollection.findOne({ "user_id": user_id });

            // Finding price of party
            const party_details = await client.db('Subteam4_Report_3').collection('party_listings').findOne({"party_id": party_id});
        ...
        try{
            ...
            // If user does have a cart
                ...
                for (let i = 0; i < parties.length; i++){
                    const party_details = await client.db('Subteam4_Report_3').collection('party_listings').findOne({"party_id": parties[i]});
                    const host_information = await client.db('Subteam4_Report_3').collection('party_host_information').findOne({"host_id": party_details.host_id});
...
async function createTransaction(request, response, party_id, user_id){
    ...
    request.on('end', async function(){
        try{
            ...
            const attendance_list = await client.db('Subteam4_Report_3').collection('attendance_list').findOne({'party_id': party_id});
            ...
            setTimeout(async () => {
                ...
                await client.db('Subteam4_Report_3').collection('transactions').deleteOne({'transaction_id': new_transaction.transaction_id}); 
                return;
            }, 900000);

            // Getting transaction data collection
            const transactions_collection = client.db('Subteam4_Report_3').collection('transactions');

            // Getting price of party (discounted or not)
            let price_of_party = 0;
            const discounted_parties = await client.db('Subteam4_Report_3').collection('discounted_parties').findOne({"party_id": party_id});
...
async function processPayment(request, response){
    ...
    request.on('end', async function(){
        try{
            ...
            const transaction = await client.db('Subteam4_Report_3').collection('transactions').findOne({'transaction_id': transaction_id});
            const party_details = await client.db('Subteam4_Report_3').collection('party_listings').findOne({'party_id': transaction.party_id});
            ...
            session = await stripe.checkout.sessions.create({
                ...
                success_url: `${process.env.SERVER_URL}/se-report3/public/html/checkoutSuccess.html`,
                cancel_url: `${process.env.SERVER_URL}/se-report3/public/html/checkoutPage.html`
            })
...
async function addToAttendanceList(response, transaction_id){
    ...
    try{
        ...
        const transaction_details = await client.db('Subteam4_Report_3').collection('transactions').findOne({'transaction_id': transaction_id});
        // Getting user_id
        const user_id = transaction_details.user_id;
        // attendance list collection
        const attendance_list_collection = client.db('Subteam4_Report_3').collection('attendance_list');
...
async function addPurchaseHistory(response, transaction_id){
    ...
    try{
        ...
        const purchase_history = client.db('Subteam4_Report_3').collection('user_purchase_history');
...
async function checkoutConfirmation(response, transaction_id){
    ...
    try {
        ...
        const transaction_details = await client.db('Subteam4_Report_3').collection('transactions').findOne({'transaction_id': transaction_id});
        const party_details = await client.db('Subteam4_Report_3').collection('party_listings').findOne({"party_id": transaction_details.party_id});
        const host_information = await client.db('Subteam4_Report_3').collection('party_host_information').findOne({"host_id": party_details.host_id});
        const user_information = await client.db('Subteam4_Report_3').collection('registered_user_profile').findOne({"user_id": transaction_details.user_id});
...
async function cancelTransaction(request, response, transaction_id){
    ...
    try {
        // Getting transaction
        const transaction_collection = client.db('Subteam4_Report_3').collection('transactions');
...
async function getTransactionHistory(request, response, user_id){
    ...
    try{
        ...
        const purchase_history_collection = client.db('Subteam4_Report_3').collection('user_purchase_history');
        const transaction_collection = client.db('Subteam4_Report_3').collection('transactions');
        const parties_collection = client.db('Subteam4_Report_3').collection('party_listings');
        const party_host_collection = client.db('Subteam4_Report_3').collection('party_host_information');
...
async function getPromotedParties(request, response){
    ...
    try{
        // Getting necessary collections/ information
        const promoted_parties_collection = client.db('Subteam4_Report_3').collection('promoted_parties');
